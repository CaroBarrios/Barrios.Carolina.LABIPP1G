...
* \param list eBicicleta* Pointer to array of bicicletas
...
/**
* \brief Add in a existing list of bicicletas the values received as parameters in the first empty position
* \param list eBicicleta* Pointer to array of bicicletas
* \param len int Array length
* \param index Position to be updated
* \param id Position where an bicicletas will be registered
...
int bicicletaPrint(eBicicleta* pElement, eTipo* listTipo, int lenTipo, eColor* listColor, int lenColor)
{
    int answer = -1;
        ...
        if(pElement != NULL && pElement->isEmpty == 0)
        {
            ...
            printf ("%3d      %20s                   %4d                        %4d                %2d   \n\n", pElement->id, pElement->marca, pElement->idTipo, pElement->idColor, pElement->rodado);
        }
    return answer;
}
